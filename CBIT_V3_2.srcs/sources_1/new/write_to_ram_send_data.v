...
wire test;
assign test = ( write_add_t == 14'd1998) ? 1'b1 : 1'b0;
assign test2 = ( sweep_cnt == 14'd1998) ? 1'b1 : 1'b0;
...
//                    end
        ...
        begin
            ...
            if(sweep_en)
                sweep_en_t <= 1'b1;
            else if(write_add_t == 12'd2000)//sweep_cnt//128个波形数据写完了，把使能关掉   地址为11到138  
                sweep_en_t <= 1'b0;
            ...
            begin
                sweep_cnt <= 14'd0;//存在ram里面的地址，前面的message已经存了10个地址了，后面的就要从11开始加
                w_sweep <= 1'b0;
            end
            ...
            begin
                ...
                begin
                    ...
                    begin
                        write_data_t <=  sweep_data; //  sweep_data[13:6]   test // 只上传了adc的高8位数据
                        sweep_cnt <= sweep_cnt + 1'b1;
                        write_add_t <= sweep_cnt;
                    end
                    else
                    begin
                        write_data_t <= sweep_data ;  // sweep_data[13:6]  test
                        write_add_t <= sweep_cnt;
                        sweep_cnt <= sweep_cnt + 1'b1;
                    end
                    extract_cnt <= 8'd0;
